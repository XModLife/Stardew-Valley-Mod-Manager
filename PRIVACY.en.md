...
Applicable Software Versions: Stardew Valley Mod Manager 1.0.0 and later versions, until this Privacy Policy is updated or replaced.
...
If automatic or manual update checking is enabled in a future release, SVMM may access:

- GitHub Releases;
- public GitHub project pages;
- software-version information endpoints designated by the developer;
- public software-download locations.

Update checks may send information required for ordinary HTTP/HTTPS communication, such as the current SVMM version, network request headers, IP address, and request time.

Unless a future release expressly states otherwise, update checking will not upload Stardew Valley save files, Mod files, user notes, categories, configuration profiles, or Nexus API Keys to developer-controlled servers.

GitHub processes request data according to its own policies.
